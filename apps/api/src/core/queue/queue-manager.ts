...
export class QueueManager implements IQueueProvider {
  ...
  private getEventBus(): IEventBus | undefined {
    try {
      ...
      logger.error(err, "Failed to resolve EventBus");
    }
    return undefined;
  }
  ...
  async createQueue(name: string): Promise<void> {
    ...
    if (eventBus) {
      Promise.resolve(eventBus.emit("queue.created", { queue: name })).catch((err: any) =>
        logger.error(err, "Failed to emit queue.created event")
      );
    }
  }
  ...
  async deleteQueue(name: string): Promise<boolean> {
    ...
    if (deleted) {
      ...
      const eventBus = this.getEventBus();
      if (eventBus) {
        Promise.resolve(eventBus.emit("queue.deleted", { queue: name })).catch((err: any) =>
          logger.error(err, "Failed to emit queue.deleted event")
        );
      }
    }
    return deleted;
  ...
  async enqueue<T>(queue: string, data: T, maxRetries?: number): Promise<Job<T>> {
    ...
    if (eventBus) {
      Promise.resolve(eventBus.emit("job.enqueued", { jobId: job.id, queue, data })).catch(
        (err: any) => logger.error(err, "Failed to emit job.enqueued event")
      );
    }
    return job;
  }
    ...
    const wrappedHandler: JobHandler<T> = async (job) => {
      if (eventBus) {
        Promise.resolve(eventBus.emit("job.started", { jobId: job.id, queue: job.name })).catch(
          (err: any) => logger.error(err, "Failed to emit job.started event")
        );
      }
      try {
        await handler(job);
        ...
        if (eventBus) {
          Promise.resolve(eventBus.emit("job.completed", { jobId: job.id, queue: job.name })).catch(
            (err: any) => logger.error(err, "Failed to emit job.completed event")
          );
        }
      } catch (err: any) {
        this.failedJobs++;
        if (eventBus) {
          ...
          ).catch((err: any) => logger.error(err, "Failed to emit job.failed event"));
        }
        throw err;
      }
    };
  ...
  async retry(jobId: string): Promise<boolean> {
    ...
    if (retried) {
      const eventBus = this.getEventBus();
      if (eventBus) {
        Promise.resolve(eventBus.emit("job.retried", { jobId })).catch((err: any) =>
          logger.error(err, "Failed to emit job.retried event")
        );
      }
    }
    return retried;
