    ...
    if (val !== undefined) {
      ...
      if (eventBus) {
        ...
        Promise.resolve(eventBus.emit("cache.hit", { key })).catch((err) => {
          logger.error({ err, key }, "Failed to emit cache hit event");
        });
      }
    } else {
      this.missCount++;
      if (eventBus) {
        Promise.resolve(eventBus.emit("cache.miss", { key })).catch((err) => {
          logger.error({ err, key }, "Failed to emit cache miss event");
        });
      }
    }
  ...
  async set<T>(key: string, value: T, ttlSeconds?: number): Promise<void> {
    ...
    if (eventBus) {
      Promise.resolve(eventBus.emit("cache.set", { key, ttlSeconds })).catch((err) => {
        logger.error({ err, key }, "Failed to emit cache set event");
      });
    }
  }
    ...
    if (deleted) {
      ...
      if (eventBus) {
        Promise.resolve(eventBus.emit("cache.deleted", { key })).catch((err) => {
          logger.error({ err, key }, "Failed to emit cache deleted event");
        });
      }
    }
    return deleted;
